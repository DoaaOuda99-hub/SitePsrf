...
const Portfolio = () => {
    const newData = data.map((data) => {
        return (
          ...
          <meta name='description' content='أنشطة وفعاليات الإتحاد الفلسطيني للشراع و التجديف'></meta>
          <link rel='canonical' href='/portfolio'></link>
            ...
            <div class="post_wrap style-2">
                  <div class="post_img">
                      <Link to="#"><img src={data.image} alt="image"/></Link>
                  </div>
                  <div class="post_info">
                  <div class="post_date"><a href="/blog-detail.html"><i class="fa fa-calendar"></i>{data.date}</a></div>
                     <h5><Link to={`/PortfolioDetail/${data.id}`}>{data.name}</Link></h5>
                      <p>{data.title}</p>
                      <div class="post_categorys">
                          <Link to={`/PortfolioDetail/${data.id}`}>تفاصيل الخبر</Link>
                      </div>
                  </div>
              </div>  
          </div>
